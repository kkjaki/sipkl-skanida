            ...
            <div class="relative sm:min-w-[180px]">
                <select name="department" onchange="this.form.submit()" aria-label="Filter Program Keahlian"
                    ...
                    @foreach ($departments as $dept)
                        <option value="{{ $dept->name }}" {{ ($filterDept ?? '') === $dept->name ? 'selected' : '' }}
                    ...
                    <tbody>
                        @forelse ($students as $index => $student)
                            <tr x-data @click="window.location.href = '{{ route('students.edit', $student->user_id) }}'"
                                ...
                                <td class="py-4 px-4">
                                    <span
                                        class="text-sm text-gray-600 dark:text-gray-300 font-mono">{{ $student->nis }}</span>
                                </td>
                                <td class="py-4 px-4">
                                    <span
                                        class="text-sm text-gray-600 dark:text-gray-300">{{ $student->class_name }}</span>
                                </td>
                                <td class="py-4 px-4">
                                ...
                                <td class="py-4 px-4 pr-8 xl:pr-8 text-right" @click.stop>
                                    <div class="flex items-center justify-end space-x-3">
                                        <a href="{{ route('students.edit', $student->user_id) }}"
                                            ...
                                            title="Edit" aria-label="Edit {{ $student->user->name }}">
                                        ...
                                        <form action="{{ route('students.destroy', $student->user_id) }}" method="POST"
                                            ...
                                            <button type="submit"
                                                ...
                                                title="Hapus" aria-label="Hapus {{ $student->user->name }}">
                                                <svg class="w-5 h-5" width="20" height="20" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center justify-end gap-4 mt-2" @click.stop>
                            ...
                            <form action="{{ route('students.destroy', $student->user_id) }}" method="POST"
                                ...
                                <button type="submit"
                                    ...
                                    aria-label="Hapus {{ $student->user->name }}">
