                ...
                <ol class="flex items-center gap-1.5 text-sm">
                    <li>
                        <a class="font-medium text-gray-400 dark:text-amoled-text hover:text-school-blue transition duration-150"
                            href="{{ route('academic-years.index') }}">Tahun Ajaran</a>
                    </li>
            ...
            <form action="{{ route('academic-years.update', $academicYear) }}" method="POST">
                ...
                <div class="p-6 sm:p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <label for="is_active" class="relative inline-flex items-center cursor-pointer gap-3"
                            x-data="{ checked: {{ old('is_active', $academicYear->is_active) ? 'true' : 'false' }} }">
                            <input type="hidden" name="is_active" value="0">
                            <div class="relative">
                                <input type="checkbox" id="is_active" name="is_active" value="1" class="sr-only peer"
                                    x-model="checked" {{ old('is_active', $academicYear->is_active) ? 'checked' : '' }} />
                                <div
                                    class="w-11 h-6 bg-gray-200 dark:bg-amoled-border rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border after:border-gray-300 dark:after:border-gray-600 after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-500">
                                </div>
                            </div>
                            <span class="text-sm text-gray-600 dark:text-gray-400"
                                x-text="checked ? 'Aktif' : 'Non-Aktif'">{{ $academicYear->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                        </label>
