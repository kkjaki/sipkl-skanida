    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 min-h-[44px]">
            ...
            <a href="{{ route('academic-years.create') }}"
                ...
                <span>
                    <svg class="fill-current w-4 h-4" width="16" height="16" viewBox="0 0 20 20" fill="none"
        ...
        @if (session('success'))
            <div
                class="flex w-full border-l-4 border-success bg-white dark:bg-amoled-surface px-4 py-3 shadow-sm rounded-r-xl">
                <div class="w-full">
        ...
        @if (session('error'))
            <div
                class="flex w-full border-l-4 border-danger bg-white dark:bg-amoled-surface px-4 py-3 shadow-sm rounded-r-xl">
                <div class="w-full">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-amoled-border dark:bg-amoled-surface">
            ...
            <div class="hidden md:block max-w-full overflow-x-auto">
                <table class="w-full table-auto">
                    <thead>
                        <tr
                            class="bg-gray-50 text-left dark:bg-white/[0.04] border-b border-gray-200 dark:border-amoled-border">
                            ...
                            <th
                                class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-right xl:pr-8">
                                Aksi
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($academicYears as $index => $year)
                            <tr
                                ...
                                <td class="py-4 px-4">
                                    @if ($year->is_active)
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold bg-emerald-500/10 text-emerald-500">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                            Aktif
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold bg-zinc-500/10 text-zinc-500">
                                    ...
                                    <div class="flex items-center justify-end space-x-3">
                                        ...
                                        @unless ($year->is_active)
                                            <form action="{{ route('academic-years.activate', $year) }}" method="POST"
                                                ...
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="text-gray-400 hover:text-emerald-500 transition duration-150 flex items-center"
                                                    title="Jadikan Tahun Ajaran Aktif">
                                                    <svg class="w-5 h-5" width="20" height="20" fill="none"
                                        ...
                                        <a href="{{ route('academic-years.edit', $year) }}"
                                            ...
                                            title="Edit" aria-label="Edit {{ $year->name }}">
                                            <svg class="w-5 h-5" width="20" height="20" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                        ...
                                        <form action="{{ route('academic-years.destroy', $year) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="text-gray-400 hover:text-danger transition duration-150 flex items-center"
                                                title="Hapus" aria-label="Hapus {{ $year->name }}">
                                                <svg class="w-5 h-5" width="20" height="20" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                        ...
                        @empty
                            <tr>
                                <td colspan="4"
                                    class="py-6 px-4 text-center text-sm text-gray-500 dark:text-amoled-text">
                        ...
                        <div class="flex items-center justify-end gap-4 mt-2">
                            @unless ($year->is_active)
                                <form action="{{ route('academic-years.activate', $year) }}" method="POST"
                                    ...
                                    @method('PATCH')
                                    <button type="submit"
                                        class="text-sm font-medium text-emerald-500 hover:text-emerald-400 flex items-center gap-1">
                            ...
                            <form action="{{ route('academic-years.destroy', $year) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="text-sm font-medium text-danger hover:text-danger/80 flex items-center gap-1"
