...
@section('content')
    <div x-data="{
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-amoled-text">
                    Daftar siswa yang sedang menjalani PKL. Gunakan fitur <span
                        class="font-semibold text-amber-600 dark:text-amber-400">Pindahkan Lokasi</span> untuk merotasi siswa
                    ke industri baru.
                </p>
            </div>
            <div class="flex-shrink-0">
                <span
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-sm font-bold border border-emerald-500/20">
        ...
        @if (session('success'))
            <div
                class="flex items-center gap-3 px-5 py-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-sm font-medium">
        ...
        @if (session('error'))
            <div
                class="flex items-center gap-3 px-5 py-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-700 dark:text-red-400 text-sm font-medium">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        {{-- Filters --}}
        <div
            class="bg-white dark:bg-amoled-surface border border-gray-200 dark:border-amoled-border rounded-3xl p-5 flex flex-col sm:flex-row gap-3">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" placeholder="Cari nama, NIS, atau kelas..."
            ...
            <button x-show="search || departmentId || industryId" x-cloak @click="search=''; departmentId=''; industryId=''"
        ...
        {{-- Table --}}
        <div
            class="bg-white dark:bg-amoled-surface border border-gray-200 dark:border-amoled-border rounded-3xl overflow-hidden">
            ...
            <template x-if="internships.length === 0">
                <div class="p-12 sm:p-16 text-center">
                    ...
                    <p class="text-sm font-semibold text-gray-400 dark:text-gray-500">Tidak ada siswa PKL aktif ditemukan.
                    </p>
                </div>
            </template>
            ...
            <template x-if="internships.length > 0 && filteredInternships.length === 0">
                <div class="p-12 sm:p-16 text-center">
                    <p class="text-sm font-semibold text-gray-400 dark:text-gray-500">Tidak ditemukan hasil sesuai filter.
                    </p>
                </div>
            </template>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-amoled-border">
                                <th
                                    class="px-5 py-4 text-left text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em]">
                                    Siswa</th>
                                @if ($departmentId === null)
                                    <th
                                        class="px-5 py-4 text-left text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em]">
                                ...
                                <th
                                    ...
                                    Industri</th>
                                <th
                                    class="px-5 py-4 text-left text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em] hidden lg:table-cell">
                                    Alamat Industri</th>
                                <th
                                    class="px-5 py-4 text-left text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em] hidden md:table-cell">
                                    Guru Pembimbing</th>
                                <th
                                    class="px-5 py-4 text-left text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em] hidden sm:table-cell">
                                    Tgl Mulai</th>
                                <th
                                    class="px-5 py-4 text-right text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em]">
                                    Aksi</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="font-bold text-gray-900 dark:text-white"
                                                    x-text="internship.student?.user?.name || '-'"></p>
                                                <p class="text-[11px] text-gray-400 dark:text-gray-500"><span
                                                        x-text="internship.student?.nis || '-'"></span> · <span
                                                        x-text="internship.student?.class_name || '-'"></span></p>
                                            </div>
                                        </div>
                                    </td>
                                            ...
                                            <span class="px-2.5 py-1 rounded-lg text-xs font-bold border"
                                                :class="{
                                                    'bg-blue-500/10 text-blue-600 border-blue-500/20 dark:bg-blue-500/20 dark:text-blue-400': (
                                                            internship.student?.department?.code || '')
                                                    .toUpperCase() === 'PPLG',
                                                    ...
                                                    'bg-emerald-500/10 text-emerald-600 border-emerald-500/20 dark:bg-emerald-500/20 dark:text-emerald-400': (
                                                            internship.student?.department?.code || '')
                                                    .toUpperCase() === 'PM',
                                                    'bg-gray-100 text-gray-600 border-gray-200 dark:bg-white/[0.06] dark:text-gray-300 dark:border-amoled-border':
                                                        !['PPLG', 'AKL', 'MPLB', 'PM'].includes((internship.student
                                    ...
                                    <td class="px-5 py-4">
                                        <p class="font-semibold text-gray-900 dark:text-white truncate max-w-[180px]"
                                            x-text="internship.industry?.name || '-'"></p>
                                        <p class="text-[11px] text-gray-400 dark:text-gray-500"
                                            x-text="internship.industry?.city || '-'"></p>
                                    </td>

                                    {{-- Alamat Industri --}}
                                    <td class="px-5 py-4 hidden lg:table-cell">
                                        <p class="text-[12px] text-gray-500 dark:text-gray-400 truncate max-w-[200px]"
                                            x-text="internship.industry?.address || '-'">
                                        </p>
                                    </td>
                                    ...
                                    <td class="px-5 py-4 hidden md:table-cell">
                                        <template x-if="internship.supervisor">
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300"
                                                x-text="internship.supervisor?.user?.name"></p>
                                        </template>
                                        <template x-if="!internship.supervisor">
                                            <span class="text-xs text-gray-400 dark:text-gray-500 italic">Belum
                                                ditetapkan</span>
                                        </template>
                                    </td>
                                    ...
                                    <td class="px-5 py-4 hidden sm:table-cell">
                                        <p class="text-sm text-gray-600 dark:text-gray-400"
                                            x-text="internship.formatted_start_date || '-'">
                                        </p>
                                    </td>
        ...
        <div x-show="withdrawModal" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <div x-show="withdrawModal" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div
                        class="p-4 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                        Status PKL siswa ini akan berubah menjadi <strong>Withdrawn</strong> dan tanggal selesai akan diisi
                        otomatis hari ini. Siswa akan kembali muncul sebagai <strong>Kandidat Pindahan</strong> di halaman
                        Plotting Siswa.
                    </div>
                </div>
