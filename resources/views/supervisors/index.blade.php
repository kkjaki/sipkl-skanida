...
@section('content')
    <div class="flex flex-col gap-6">
        ...
        <!-- Search & Filters -->
        <form method="GET" action="{{ route('supervisors.index') }}"
                ...
                <select name="department" onchange="this.form.submit()" title="Filter Program Keahlian"
                    ...
                    @foreach ($departments as $dept)
                        <option value="{{ $dept->id }}" {{ ($filterDept ?? '') == $dept->id ? 'selected' : '' }}
                            class="dark:bg-amoled-surface">{{ $dept->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="relative sm:min-w-[160px]">
                <select name="role" onchange="this.form.submit()" title="Filter Jabatan"
                    ...
                    <option value="department_head" {{ ($filterRole ?? '') === 'department_head' ? 'selected' : '' }}
                        class="dark:bg-amoled-surface">Kaprog</option>
                    <option value="supervisor" {{ ($filterRole ?? '') === 'supervisor' ? 'selected' : '' }}
                        class="dark:bg-amoled-surface">Pembimbing</option>
                </select>
                <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                    <svg class="w-4 h-4 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                    ...
                    <thead>
                        <tr class="border-b border-gray-200 dark:border-amoled-border">
                            ...
                            <th class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-left">
                                Nama</th>
                            <th class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-left">NIP
                            </th>
                            <th class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-left">
                                Program Keahlian</th>
                            <th class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-left">
                        ...
                        @forelse($supervisors as $index => $supervisor)
                            <tr x-data @click="window.location.href = '{{ route('supervisors.edit', $supervisor->user_id) }}'" class="hover:bg-gray-50 dark:hover:bg-white/[0.03] transition duration-150 cursor-pointer">
                                ...
                                <td class="py-3 px-4">
                                    ...
                                    @else
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border bg-school-blue/10 text-school-blue border-school-blue/20 dark:bg-school-blue/20 dark:text-blue-400 dark:border-blue-500/30">
                                            Pembimbing
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="py-3 px-4" @click.stop>
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('supervisors.edit', $supervisor->user_id) }}"
                                            ...
                                            title="Edit" aria-label="Edit {{ $supervisor->user->name }}">
                                        ...
                                        <form action="{{ route('supervisors.destroy', $supervisor->user_id) }}"
                                            method="POST"
                                            @submit.prevent="$store.confirmModal.show({ title: 'Hapus Guru Pembimbing', message: 'Apakah Anda yakin ingin menghapus guru pembimbing ini?', type: 'danger', confirmText: 'Ya, Hapus', onConfirm: () => $el.submit() })">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                ...
                                                title="Hapus" aria-label="Hapus {{ $supervisor->user->name }}">
                                                <svg class="w-4 h-4" width="16" height="16" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div x-data @click="window.location.href = '{{ route('supervisors.edit', $supervisor->user_id) }}'"
                    ...
                    <div class="flex items-start justify-between mb-2">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 dark:text-gray-500 font-mono mt-0.5">NIP:
                                {{ $supervisor->nip }}</p>
                        </div>
                        @if ($supervisor->user->hasRole('department_head'))
                            <span
                    ...
                    <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-amoled-border" @click.stop>
                        ...
                        <form action="{{ route('supervisors.destroy', $supervisor->user_id) }}" method="POST"
                            @submit.prevent="$store.confirmModal.show({ title: 'Hapus Guru Pembimbing', message: 'Apakah Anda yakin ingin menghapus guru pembimbing ini?', type: 'danger', confirmText: 'Ya, Hapus', onConfirm: () => $el.submit() })">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex items-center gap-1.5 text-xs font-medium text-red-500 hover:underline"
                                aria-label="Hapus {{ $supervisor->user->name }}">
