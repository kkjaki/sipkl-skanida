                ...
                <ol class="flex items-center gap-1.5 text-sm">
                    <li>
                        <a class="font-medium text-gray-400 dark:text-amoled-text hover:text-school-blue transition duration-150"
                            href="{{ route('supervisors.index') }}">Guru Pembimbing</a>
                    </li>
                    <li class="text-gray-300 dark:text-gray-600">/</li>
                    <li class="font-medium text-gray-800 dark:text-gray-200">Tambah Guru Pembimbing Baru</li>
                </ol>
            </nav>
        </div>
                ...
                <div class="p-6 sm:p-8 space-y-6 border-b border-gray-200 dark:border-amoled-border">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}"
                            placeholder="cth. Budi Santoso, S.Pd." required
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-xs text-gray-400 dark:text-gray-500">
                            <svg class="w-3.5 h-3.5 inline-block mr-0.5 -mt-0.5" width="14" height="14"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            Kosongkan untuk auto-generate email sekolah (<span
                                class="font-mono">NIP@smkn2magelang.sch.id</span>).
                        </p>
                        @error('email')
                            <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="rounded-xl bg-blue-500/5 border border-blue-500/10 px-4 py-3">
                        <p class="text-xs text-blue-500 dark:text-blue-400 font-medium flex items-center gap-1.5">
                            ...
                            Password default adalah NIP guru pembimbing.
                        </p>
                    </div>
                </div>
                ...
                <div class="p-6 sm:p-8 space-y-6 border-b border-gray-200 dark:border-amoled-border">
                    ...
                    <div>
                        ...
                        <input type="text" id="nip" name="nip" value="{{ old('nip') }}"
                            placeholder="cth. 198507012010011001" required
                            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 outline-none transition duration-150 focus:border-school-blue focus:ring-3 focus:ring-school-blue/10 dark:border-amoled-border dark:bg-amoled-input dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-school-blue" />
                        @error('nip')
                            <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
                    ...
                    <div x-data="{ isDeptHead: {{ old('is_department_head') ? 'true' : 'false' }} }">
                        <label
                            class="flex items-center justify-between cursor-pointer rounded-xl border border-gray-200 dark:border-amoled-border p-4 transition hover:bg-gray-50 dark:hover:bg-white/[0.03]">
                            <div class="flex items-center gap-3">
                                <div
                                    class="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-500/10 dark:bg-amber-500/20 shrink-0">
                                    <svg class="w-4.5 h-4.5 text-amber-500" width="18" height="18" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M5 3l3.057-3L12 3.5 15.943 0 19 3l-3 6h-8L5 3zM12 12l-4 9h8l-4-9z"></path>
                                    </svg>
                                </div>
                                <div>
                                    <p class="text-sm font-medium text-gray-800 dark:text-white">Jadikan Kepala Program
                                        Keahlian</p>
                                    <p class="text-xs text-gray-400 dark:text-gray-500">Guru ini akan memiliki hak akses
                                        sebagai Kaprog</p>
                                </div>
                            </div>
                            <div class="relative">
                                ...
                                <input type="checkbox" name="is_department_head" value="1" x-model="isDeptHead"
                                    class="sr-only peer" />
                                <div
                                    class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-white/10 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-500">
                                </div>
                            </div>
                        </label>
                        @error('is_department_head')
