...
@section('content')
    <div x-data="{
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Jurnal tanggal <span
                            class="font-medium text-school-blue">{{ $journal->date->translatedFormat('d F Y') }}</span>
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="p-4 sm:p-5 rounded-2xl bg-red-50 dark:bg-red-500/5 border border-red-200 dark:border-red-500/20">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-red-600 dark:text-red-300 mt-1 leading-relaxed">
                            {{ $journal->rejection_note }}</p>
                        <p class="text-xs text-red-500/70 dark:text-red-400/50 mt-2">Perbaiki jurnal sesuai catatan di atas,
                            lalu kirim ulang.</p>
                    </div>
                </div>
            </div>
        ...
        @if ($errors->any())
            <div
                class="p-4 text-sm text-red-800 border border-red-300 rounded-xl bg-red-50 dark:bg-red-500/10 dark:text-red-400 dark:border-red-800/50">
        ...
        {{-- Edit Form --}}
        <div
            class="bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-3xl overflow-hidden">
            <div class="p-5 sm:p-6 border-b border-gray-100 dark:border-amoled-border">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Setelah disimpan, status validasi akan kembali ke
                    <span class="font-semibold text-yellow-600 dark:text-yellow-500">Pending</span>.</p>
            </div>

            <form action="{{ route('student.journals.update', $journal) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <label class="relative cursor-pointer" @click="attendance = 'sick'">
                            <input type="radio" name="status_attendance" value="sick" x-model="attendance"
                                class="sr-only peer">
                            <div
                                class="flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all duration-200
                        ...
                        <label class="relative cursor-pointer" @click="attendance = 'excused'">
                            <input type="radio" name="status_attendance" value="excused" x-model="attendance"
                                class="sr-only peer">
                            <div
                                class="flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all duration-200
                ...
                <div x-show="attendance === 'present'" x-cloak>
                    <label for="activity"
                        class="block text-sm font-semibold text-gray-900 dark:text-white mb-1.5">Kegiatan Hari Ini <span
                ...
                <div>
                    ...
                    @if ($journal->attachment_path)
                        <div
                            class="mb-2 p-3 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-amoled-border flex items-center justify-between">
                            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 min-w-0">
                                <svg class="w-4 h-4 flex-shrink-0 text-school-blue" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <a href="{{ Storage::url($journal->attachment_path) }}" target="_blank"
                                class="text-xs font-bold text-school-blue hover:underline flex-shrink-0">Lihat</a>
                        </div>
                    @endif

                    <label
                        class="flex flex-col items-center justify-center w-full h-32 rounded-2xl border-2 border-dashed transition-all duration-200 cursor-pointer
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6" x-show="!fileName">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400"><span
                                    class="font-semibold text-school-blue">Klik untuk ganti file</span></p>
                            <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">JPG, PNG, atau PDF (Maks. 2MB)</p>
                        </div>
                        <div class="flex items-center gap-2 text-sm text-school-blue font-medium" x-show="fileName"
                            x-cloak>
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <input type="file" name="attachment_path" class="hidden" accept=".jpg,.jpeg,.png,.pdf"
