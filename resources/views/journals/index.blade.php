...
<div x-data="{
    ...
    {{-- ============================================================ --}}
    {{-- SECTION 1: FORM JURNAL --}}
    {{-- ============================================================ --}}

    @if(!$internship)
        {{-- No Active Internship Warning --}}
        <div class="flex flex-col items-center justify-center py-16 bg-white dark:bg-amoled-surface border border-dashed border-gray-200 dark:border-amoled-border rounded-3xl">
    ...
    @else
        ...
        <div class="bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-3xl overflow-hidden">
            <div class="p-5 sm:p-6 border-b border-gray-100 dark:border-amoled-border">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Lengkapi presensi dan kegiatan harian Anda.</p>
            </div>

            <form action="{{ route('student.journals.store') }}" method="POST" enctype="multipart/form-data" class="p-5 sm:p-6 space-y-5">
                @csrf
                ...
                <div class="pt-2">
                    <button type="submit"
                            class="w-full sm:w-auto py-3 px-8 rounded-2xl bg-school-blue hover:bg-school-blue/90 text-white font-bold text-sm transition-all shadow-lg shadow-school-blue/10 flex items-center justify-center gap-2 active:scale-[0.98]">
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            ...
            @if($journals->total() > 0)
                <span class="px-3 py-1 rounded-xl bg-gray-100 dark:bg-white/5 text-xs font-bold text-gray-500 dark:text-gray-400">
        ...
        @if($journals->count() > 0)
            <div class="space-y-3">
                @foreach($journals as $journal)
                    <div class="bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-2xl sm:rounded-3xl overflow-hidden transition-all duration-200 hover:shadow-lg hover:shadow-gray-100/50 dark:hover:shadow-none">
                        <div class="p-4 sm:p-5">
                            {{-- Top Row: Date + Status Badge --}}
                            <div class="flex items-start justify-between gap-3 mb-3">
                                <div class="flex items-center gap-3 min-w-0">
                                    {{-- Date Circle --}}
                                    <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl flex flex-col items-center justify-center flex-shrink-0
                                                ...
                                                @endif">
                                        <span class="text-sm sm:text-base font-black leading-none">{{ $journal->date->format('d') }}</span>
                                ...
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    @if($journal->verification_status === 'pending')
                                        <span class="px-2.5 py-1 rounded-lg bg-yellow-100 text-yellow-700 dark:bg-yellow-500/15 dark:text-yellow-500 text-[10px] font-bold uppercase tracking-wider">Pending</span>
                                    @elseif($journal->verification_status === 'verified')
                                        <span class="px-2.5 py-1 rounded-lg bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-500 text-[10px] font-bold uppercase tracking-wider">Terverifikasi</span>
                                    @else
                                        <span class="px-2.5 py-1 rounded-lg bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-500 text-[10px] font-bold uppercase tracking-wider">Ditolak</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            @if($journal->verification_status === 'rejected' && $journal->rejection_note)
                                <div class="mt-3 ml-14 sm:ml-15 p-3 rounded-xl bg-red-50 dark:bg-red-500/5 border border-red-200 dark:border-red-500/20">
                            ...
                            @if(in_array($journal->verification_status, ['pending', 'rejected']))
                                <div class="mt-3 pl-14 sm:pl-15">
                                    <a href="{{ route('student.journals.edit', $journal) }}"
                                       class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all duration-200
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center py-16 bg-white dark:bg-amoled-surface border border-dashed border-gray-200 dark:border-amoled-border rounded-3xl">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center mt-1">Mulai isi jurnal harian Anda dengan menggunakan form di atas.</p>
            </div>
        @endif
    </div>
    @endif
