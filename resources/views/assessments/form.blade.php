    ...
    <div x-data="{
        ...
        <div
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <h1 class="text-lg font-bold text-gray-900 dark:text-white truncate">Input Nilai —
        ...
        {{-- Validation Errors --}}
        @if ($errors->any())
            <div class="p-3 text-sm text-red-800 border border-red-200 rounded-xl bg-red-50 dark:bg-red-500/10 dark:text-red-400 dark:border-red-800/40"
                role="alert">
        ...
        @if ($evaluationIndicators->isEmpty())
            <div
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center">Hubungi WKS Kurikulum untuk
                    menambahkan indikator penilaian terlebih dahulu.</p>
            </div>
        @else
                ...
                <div
                    ...
                    <div class="hidden sm:block overflow-x-auto">
                        <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
                                ...
                                @foreach ($evaluationIndicators as $index => $indicator)
                                    ...
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td class="px-5 py-3.5">
                                            <input type="hidden" name="scores[{{ $indicator->id }}][indicator_id]"
                                                value="{{ $indicator->id }}">
                                            <input type="number" name="scores[{{ $indicator->id }}][industry_score]"
                                                x-model.number="scores[{{ $indicator->id }}].industry" min="0"
                                        ...
                                        <td class="px-5 py-3.5">
                                            <input type="number" name="scores[{{ $indicator->id }}][supervisor_score]"
                                                x-model.number="scores[{{ $indicator->id }}].school" min="0"
                                                max="100" step="1" placeholder="0-100"
                                                class="w-full max-w-[120px] mx-auto block text-center rounded-xl border border-gray-200 dark:border-amoled-border bg-gray-50 dark:bg-white/[0.03] text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 p-2.5 text-sm focus:ring-2 focus:ring-school-blue/20 focus:border-school-blue transition-colors">
                                        </td>
                                        <td class="px-5 py-3.5 text-center">
                                            <span
                                                class="inline-flex items-center justify-center min-w-[48px] px-2.5 py-1 rounded-lg text-sm font-bold"
                            ...
                            <tfoot
                                class="border-t-2 border-gray-200 dark:border-amoled-border bg-gray-50/80 dark:bg-white/[0.02]">
                                <tr>
                                    <td colspan="4"
                                        class="px-5 py-3.5 text-right font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wide">
                                        Rata-Rata Keseluruhan</td>
                                    <td class="px-5 py-3.5 text-center">
                                        <span
                                            class="inline-flex items-center justify-center min-w-[48px] px-3 py-1.5 rounded-xl text-sm font-black"
                                            :class="avgClass(grandAvg())" x-text="grandAvg()"></span>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                    ...
                    <div class="sm:hidden divide-y divide-gray-100 dark:divide-amoled-border">
                            ...
                            <div class="p-4 space-y-3">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex items-center gap-2.5 min-w-0">
                                        <span
                                            class="w-7 h-7 rounded-lg bg-school-blue/10 flex items-center justify-center text-school-blue text-xs font-bold flex-shrink-0">{{ $index + 1 }}</span>
                                        <span
                                            class="text-sm font-bold text-gray-900 dark:text-white truncate">{{ $indicator->name }}</span>
                                    </div>
                                ...
                                <div class="grid grid-cols-2 gap-2.5">
                                    <div>
                                        <label
                                            class="block text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Nilai
                                            Industri</label>
                                        <input type="number" name="scores[{{ $indicator->id }}][industry_score]"
                                            x-model.number="scores[{{ $indicator->id }}].industry" min="0"
                                            max="100" step="1" placeholder="0-100"
                        ...
                        <div class="p-4 bg-gray-50/80 dark:bg-white/[0.02]">
                            <div class="flex items-center justify-between">
                                <span
                                    class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wide">Rata-Rata</span>
                                <span class="px-3 py-1 rounded-xl text-sm font-black" :class="avgClass(grandAvg())"
        ...
        {{-- Confirmation Modal --}}
        <div x-show="showConfirm" class="fixed inset-0 z-[100]" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center justify-center min-h-screen p-4 sm:p-6">
                ...
                <div class="relative w-full sm:max-w-sm bg-white dark:bg-amoled-surface rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-amoled-border"
                    ...
                    <div class="p-5 sm:p-6 text-center">
                        <div
                            class="w-12 h-12 rounded-full bg-amber-100 dark:bg-amber-500/15 flex items-center justify-center mx-auto mb-4">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">Rata-rata keseluruhan: <span
                                class="font-bold text-gray-900 dark:text-white" x-text="grandAvg()"></span></p>
                        <p class="text-xs text-gray-400 dark:text-gray-500">Pastikan semua nilai sudah benar sebelum
                            menyimpan.</p>
