...
<div x-data="{
    ...
    @if($internships->isEmpty())
        <div class="flex flex-col items-center justify-center py-16 sm:py-20 bg-white dark:bg-amoled-surface border border-dashed border-gray-200 dark:border-amoled-border rounded-3xl">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center mt-1">Anda belum memiliki siswa yang di-plot sebagai bimbingan.</p>
        </div>
    @else
        ...
        <div x-show="filtered.length === 0" x-cloak
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center mt-1 mb-4">Siswa dengan filter tersebut tidak ditemukan.</p>
            <button @click="search = ''; status = ''" class="text-sm font-bold text-school-blue hover:text-school-blue/80">Reset Filter</button>
        </div>
                   ...
                   class="group flex flex-col bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-xl hover:shadow-school-blue/5 hover:-translate-y-0.5">
                    <div class="p-4 sm:p-5 flex-1">
                        <div class="flex justify-between items-start gap-2 mb-3">
                            ...
                            @else
                                <span class="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-gray-400">
                                    Belum Dinilai
                                </span>
                            @endif
                        </div>
