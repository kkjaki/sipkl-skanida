    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 min-h-[44px]">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-amoled-text mt-0.5">Kelola dokumen kerjasama seluruh industri mitra
                </p>
            </div>
        </div>

        <!-- Search & Filters -->
        <form method="GET" action="{{ route('partnerships.index') }}"
            class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <!-- Search -->
            <div class="relative flex-1">
                <span class="absolute inset-y-0 left-0 flex items-center pl-3.5">
                    <svg class="w-5 h-5 text-gray-400 dark:text-gray-500" width="20" height="20" fill="none"
            ...
            <div class="relative sm:min-w-[180px]">
                <select name="filter" onchange="this.form.submit()" title="Filter Status"
                    ...
                    <option value="active" {{ $filter === 'active' ? 'selected' : '' }} class="dark:bg-amoled-surface">MoU
                        Aktif ({{ $countActive }})</option>
                    <option value="expiring" {{ $filter === 'expiring' ? 'selected' : '' }} class="dark:bg-amoled-surface">
                        Segera Berakhir ({{ $countExpiring }})</option>
                    <option value="expired" {{ $filter === 'expired' ? 'selected' : '' }} class="dark:bg-amoled-surface">
                        Expired ({{ $countExpired }})</option>
                    <option value="none" {{ $filter === 'none' ? 'selected' : '' }} class="dark:bg-amoled-surface">Belum
                        Ada MoU ({{ $countNone }})</option>
                </select>
                ...
                <table class="w-full table-auto">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-amoled-border">
                        @forelse($industries as $index => $industry)
                            @php
                                $activePartnership = $industry->partnerships->first(
                                    fn($p) => now()->betweenIncluded(
                                        $p->start_date->startOfDay(),
                                        $p->end_date->endOfDay(),
                                    ),
                                );
                                $upcomingPartnership = $industry->partnerships->first(
                                    fn($p) => $p->start_date->startOfDay() > now()->startOfDay(),
                                );
                            ...
                            <tr x-data @click="window.location.href = '{{ route('partnerships.manage', $industry->id) }}'" class="hover:bg-gray-50 dark:hover:bg-white/[0.03] transition duration-150 cursor-pointer">
                                ...
                                <td class="py-3 px-4">
                                    ...
                                    @if ($industry->partnerships_count > 0)
                                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                                            {{ $industry->partnerships_count }} dokumen MoU</p>
                                    @endif
                                </td>
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-300">
                                    {{ $industry->city }}
                                ...
                                <td class="py-3 px-4">
                                    @if ($upcomingPartnership)
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border bg-blue-500/10 text-blue-600 border-blue-500/20 dark:bg-blue-500/20 dark:text-blue-400 dark:border-blue-500/30">
                                            Akan Datang
                                        </span>
                                    @elseif($activePartnership)
                                        @if ($activePartnership->is_expiring_soon)
                                            <span
                                                class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border bg-amber-500/10 text-amber-600 border-amber-500/20 dark:bg-amber-500/20 dark:text-amber-400 dark:border-amber-500/30">
                                    ...
                                    @else
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border bg-gray-500/10 text-gray-500 border-gray-500/20 dark:bg-gray-500/20 dark:text-gray-400 dark:border-gray-500/30">
                                            Belum Ada MoU
                                        </span>
                                    @endif
                                </td>
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-300">
                                    @if ($upcomingPartnership)
                                        <span
                                            class="text-blue-600 dark:text-blue-400">{{ $upcomingPartnership->start_date->format('d M Y') }}
                                            – {{ $upcomingPartnership->end_date->format('d M Y') }}</span>
                                    @elseif($activePartnership)
                                        <span>{{ $activePartnership->start_date->format('d M Y') }} –
                                            {{ $activePartnership->end_date->format('d M Y') }}</span>
                                    @elseif($latestPartnership)
                                        <span
                                            class="text-gray-400 dark:text-gray-500 line-through">{{ $latestPartnership->start_date->format('d M Y') }}
                                            – {{ $latestPartnership->end_date->format('d M Y') }}</span>
                                    @else
        ...
        <div class="flex flex-col gap-3 sm:hidden">
            @forelse($industries as $industry)
                @php
                    $activePartnership = $industry->partnerships->first(
                        fn($p) => now()->betweenIncluded($p->start_date->startOfDay(), $p->end_date->endOfDay()),
                    );
                    $upcomingPartnership = $industry->partnerships->first(
                        fn($p) => $p->start_date->startOfDay() > now()->startOfDay(),
                    );
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                        @if ($upcomingPartnership)
                            <span
                                ...
                                – {{ $upcomingPartnership->end_date->format('d M Y') }}</span>
                        @elseif($activePartnership)
                            {{ $activePartnership->start_date->format('d M Y') }} –
                            {{ $activePartnership->end_date->format('d M Y') }}
                        @elseif($latestPartnership)
                            <span class="line-through">{{ $latestPartnership->start_date->format('d M Y') }} –
                                {{ $latestPartnership->end_date->format('d M Y') }}</span>
                        @else
