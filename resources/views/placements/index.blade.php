    ...
    <div x-data="{
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                class="flex items-center p-4 text-sm text-green-800 border border-green-300 rounded-xl bg-green-50 dark:bg-green-500/10 dark:text-green-400 dark:border-green-800/50"
                role="alert">
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                class="flex items-center p-4 text-sm text-red-800 border border-red-300 rounded-xl bg-red-50 dark:bg-red-500/10 dark:text-red-400 dark:border-red-800/50"
                role="alert">
                <svg class="flex-shrink-0 w-4 h-4 mr-3" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                    <path
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="industrySearch"
            ...
            <select x-model="filterCity" title="Filter Kota"
                class="h-11 rounded-xl border border-gray-200 bg-white pl-4 pr-10 py-2.5 text-sm text-gray-800 outline-none transition duration-150 focus:border-school-blue focus:ring-3 focus:ring-school-blue/10 dark:border-amoled-border dark:bg-amoled-surface dark:text-white/90 dark:focus:border-school-blue cursor-pointer min-w-[160px]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            <template x-for="industry in filteredIndustries" :key="industry.id">
                <div
                    class="group flex flex-col bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-3xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-school-blue/5 hover:-translate-y-1">
                    <div class="p-5 sm:p-6 flex-1">
                            ...
                            <div class="p-2.5 sm:p-3 rounded-2xl bg-school-blue/5 dark:bg-school-blue/10 text-school-blue">
                                <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                        d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H5m14 0h2m-16 0H3m2 0V5m12 0v16m-5-14h.01M9 7h.01M9 11h.01M12 11h.01M9 15h.01M12 15h.01" />
                                </svg>
                            </div>
                            <span
                                :class="industry.remaining_quota > 0 ?
                        ...
                        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-1 flex items-start gap-1.5">
                            ...
                            <span class="line-clamp-2"
                                x-text="(industry.address ? industry.address + ', ' : '') + industry.city"></span>
                        </p>
                    ...
                    <div class="px-5 sm:px-6 py-3 sm:py-4 border-t border-gray-100 dark:border-amoled-border">
                        <div
                            :title="!industry.has_active_mou ?
                                'Industri ini tidak dapat di-plot karena tidak memiliki MoU aktif.' : ''">
        ...
        <template x-if="industries.length === 0">
            <div
                class="flex flex-col items-center justify-center py-16 sm:py-20 bg-white dark:bg-amoled-surface border border-dashed border-gray-200 dark:border-amoled-border rounded-3xl">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center">Belum ada industri yang
                    dialokasikan untuk program keahlian Anda di tahun ajaran ini.</p>
            </div>
        </template>
        ...
        <template x-if="industries.length > 0 && filteredIndustries.length === 0">
            <div
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center">Tidak ada industri yang cocok
                    dengan pencarian atau filter Anda.</p>
            </div>
        </template>
        ...
        <div x-show="showModal" class="fixed inset-0 z-[100] overflow-y-auto"
            ...
            <div class="flex items-center justify-center min-h-screen p-4 sm:p-6">
                ...
                <div class="relative w-full sm:max-w-2xl bg-white dark:bg-amoled-surface rounded-2xl sm:rounded-[2rem] shadow-2xl overflow-hidden border border-gray-200 dark:border-amoled-border max-h-[90vh] flex flex-col"
                    ...
                    <!-- Modal Header -->
                    <div
                        class="p-5 sm:p-8 border-b border-gray-100 dark:border-amoled-border flex items-start justify-between gap-3 flex-shrink-0">
                        <div class="min-w-0 flex-1">
                            <h2 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate"
                                x-text="selectedIndustry ? selectedIndustry.name : ''"></h2>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-1"
                                x-text="selectedIndustry ? ((selectedIndustry.address ? selectedIndustry.address + ', ' : '') + selectedIndustry.city) : ''">
                            </p>
                            <div class="flex items-center gap-3 mt-2">
                                <span class="text-xs font-bold text-school-blue uppercase tracking-wider"
                                    x-text="'Sisa Kuota: ' + limit + ' Siswa'"></span>
                            </div>
                        </div>
                        <button @click="closeModal()"
                            class="p-2 rounded-xl bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-white hover:bg-gray-200 dark:hover:bg-white/20 transition-all flex-shrink-0">
                            <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                    ...
                    <div class="p-5 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                        ...
                        <div class="mb-8 sm:mb-10">
                            <div class="flex items-center justify-between mb-3 sm:mb-4">
                                <h3
                                    class="text-[11px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.2em]">
                                    Siswa Terplot</h3>
                                <div class="flex items-center gap-2">
                                    <template x-if="selectedIndustry && selectedIndustry.current_interns.length > 0">
                                        <button @click="deleteMode = !deleteMode; selectedInterns = []"
                                            ...
                                            :class="deleteMode ? 'bg-red-500/10 text-red-500' :
                                                'bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400 hover:bg-red-500/10 hover:text-red-500'">
                                            <span x-text="deleteMode ? 'Batal' : 'Hapus Bulk'"></span>
                                        </button>
                                    </template>
                                    <span
                                        class="px-2 py-0.5 rounded-lg bg-gray-100 dark:bg-white/5 text-[10px] font-bold text-gray-500 dark:text-gray-400"
                                        x-text="selectedIndustry ? selectedIndustry.current_interns.length : 0"></span>
                                </div>
                            </div>

                            <!-- Select All (bulk delete mode) -->
                            <div x-show="deleteMode && selectedIndustry && selectedIndustry.current_interns.length > 1"
                                class="mb-3" x-cloak>
                                <label @click="toggleAllInterns()"
                                    class="flex items-center gap-3 p-3 rounded-xl bg-red-500/5 dark:bg-red-500/10 border border-red-500/20 cursor-pointer">
                                    <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-all"
                                        :class="selectedIndustry && selectedInterns.length === selectedIndustry.current_interns
                                            .length ? 'bg-red-500 border-red-500' :
                                            'border-gray-300 dark:border-amoled-border'">
                                        <svg x-show="selectedIndustry && selectedInterns.length === selectedIndustry.current_interns.length"
                            ...
                            <div class="grid grid-cols-1 gap-2 sm:gap-3">
                                <template x-if="selectedIndustry && selectedIndustry.current_interns.length > 0">
                                    <template x-for="intern in selectedIndustry.current_interns" :key="intern.id">
                                        <div
                                            class="flex items-center justify-between p-3 sm:p-4 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-amoled-border group/intern">
                                            <div class="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                                                ...
                                                <template x-if="deleteMode">
                                                    <div @click="toggleInternDelete(intern.id)"
                                                        ...
                                                        :class="selectedInterns.includes(intern.id) ?
                                                            'bg-red-500 border-red-500' :
                                                            'border-gray-300 dark:border-amoled-border hover:border-red-400'">
                                                        <svg x-show="selectedInterns.includes(intern.id)"
                                                ...
                                                <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-school-blue/10 flex items-center justify-center text-school-blue font-black text-sm sm:text-lg flex-shrink-0"
                                                    x-text="intern.student.user.name.charAt(0)"></div>
                                                <div class="min-w-0">
                                                    <p class="text-sm font-bold text-gray-900 dark:text-white truncate"
                                                        x-text="intern.student.user.name"></p>
                                                    <p class="text-[11px] font-medium text-gray-500 dark:text-gray-400 truncate"
                                                        x-text="intern.student.class_name + (intern.student.address ? ' • ' + intern.student.address : '')">
                                                    </p>
                                                </div>
                                            </div>
                                            <!-- Single delete button (non-bulk mode) -->
                                            <template x-if="!deleteMode">
                                                <form :action="'{{ url('placements') }}/' + intern.id" method="POST"
                                                    @submit.prevent="$store.confirmModal.show({ title: 'Hapus Penempatan', message: 'Hapus penempatan siswa ini?', type: 'danger', confirmText: 'Ya, Hapus', onConfirm: () => $el.submit() })">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="p-2 sm:p-2.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/15 rounded-xl transition-all opacity-0 group-hover/intern:opacity-100 flex-shrink-0">
                                ...
                                <template x-if="!selectedIndustry || selectedIndustry.current_interns.length === 0">
                                    <div
                                        class="p-6 sm:p-8 text-center border-2 border-dashed border-gray-100 dark:border-amoled-border rounded-3xl">
                                        <p class="text-sm text-gray-400 dark:text-gray-500 italic font-medium">Belum ada
                                            siswa yang di-plot ke industri ini.</p>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div x-show="deleteMode && selectedInterns.length > 0" class="mt-3" x-cloak>
                                <form action="{{ route('placements.destroyBulk') }}" method="POST"
                                    @submit.prevent=".confirmModal.show({ title: 'Hapus Penempatan Massal', message: 'Hapus ' + $el.querySelector('[data-count]').dataset.count + ' penempatan siswa? Siswa akan kembali ke daftar kandidat.', type: 'danger', confirmText: 'Ya, Hapus Semua', onConfirm: () => $el.submit() })">
                                    @csrf
                                    @method('DELETE')
                                    <template x-for="id in selectedInterns" :key="'del-' + id">
                                        <input type="hidden" name="internship_ids[]" :value="id">
                                    </template>
                                    <button type="submit" :data-count="selectedInterns.length" data-count=""
                        ...
                        <div x-show="limit > 0">
                            <div class="flex items-center justify-between mb-3 sm:mb-4">
                                <h3
                                    ...
                                    Pilih Kandidat</h3>
                                <span
                                    class="px-2 py-0.5 rounded-lg bg-school-blue/10 text-[10px] font-bold text-school-blue"
                            ...
                            <div class="grid grid-cols-1 gap-2 sm:gap-3">
                                ...
                                <template x-if="regularCandidates.length > 0">
                                    <p
                                        class="text-[10px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.2em] px-1">
                                        Siswa Reguler</p>
                                </template>
                                <template x-for="student in regularCandidates" :key="student.user_id">
                                    <label
                                        class="relative flex items-center p-3 sm:p-4 rounded-2xl border transition-all cursor-pointer group/label"
                                        :class="[
                                            ...
                                            isDisabled(student.user_id) ?
                                            'opacity-30 cursor-not-allowed filter grayscale' : ''
                                        ]">
                                        <input type="checkbox" class="sr-only" :value="student.user_id"
                                        ...
                                        <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                                            <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 dark:text-gray-400 font-black text-sm sm:text-base flex-shrink-0"
                                                :class="selectedStudents.includes(student.user_id) ?
                                                    'bg-school-blue/20 text-school-blue' : ''"
                                                x-text="student.user.name.charAt(0)"></div>
                                            <div class="min-w-0">
                                                <p class="text-sm font-bold text-gray-900 dark:text-white truncate"
                                                    x-text="student.user.name"></p>
                                                <p class="text-[11px] font-medium text-gray-500 dark:text-gray-400 truncate"
                                                    x-text="student.class_name + (student.address ? ' • ' + student.address : '')">
                                                </p>
                                            </div>
                                        </div>
                                        <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 flex-shrink-0"
                                            :class="selectedStudents.includes(student.user_id) ?
                                                'bg-school-blue border-school-blue scale-110' :
                                                'border-gray-300 dark:border-amoled-border group-hover/label:border-school-blue/50'">
                                            <svg x-show="selectedStudents.includes(student.user_id)"
                                                class="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                                    d="M5 13l4 4L19 7" />
                                            </svg>
                                ...
                                <template x-if="transferCandidates.length > 0">
                                    <div class="flex items-center gap-3 py-1">
                                        ...
                                        <span
                                            class="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-[0.2em] flex items-center gap-1.5">
                                    ...
                                    <label
                                            ...
                                            <div class="min-w-0">
                                                <div class="flex items-center gap-2 flex-wrap">
                                                    <p class="text-sm font-bold text-gray-900 dark:text-white truncate"
                                                        x-text="student.user.name"></p>
                                                    <span
                                                        class="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 flex-shrink-0">Pindahan</span>
                                                </div>
                                                <p class="text-[11px] font-medium text-gray-500 dark:text-gray-400 truncate"
                                                    x-text="student.class_name + (student.address ? ' • ' + student.address : '')">
                                                </p>
                                        ...
                                        <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 flex-shrink-0"
                                            :class="selectedStudents.includes(student.user_id) ?
                                                ...
                                                'border-amber-300 dark:border-amber-500/40 group-hover/label:border-amber-400'">
                                            <svg x-show="selectedStudents.includes(student.user_id)"
                                                class="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                ...
                                <template x-if="candidates.length === 0">
                                    <div
                                        class="p-6 sm:p-8 text-center border-2 border-dashed border-gray-100 dark:border-amoled-border rounded-3xl">
                                        <p class="text-sm text-gray-400 dark:text-gray-500 italic font-medium">Semua siswa
                                            sudah memiliki penempatan.</p>
                                    </div>
                                </template>
                                ...
                                <template x-if="candidates.length > 0 && filteredCandidates.length === 0">
                                    <div
                                        class="p-6 sm:p-8 text-center border-2 border-dashed border-gray-100 dark:border-amoled-border rounded-3xl">
                                        <p class="text-sm text-gray-400 dark:text-gray-500 italic font-medium">Tidak
                                            ditemukan kandidat yang cocok.</p>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div x-show="hasSelectedTransfer" x-cloak
                                ...
                                <label
                                    class="block text-xs font-black text-amber-700 dark:text-amber-400 uppercase tracking-[0.15em] mb-2">
                                ...
                                <input type="date" name="transfer_start_date" x-model="transferStartDate"
                        ...
                        <template
                            x-if="limit <= 0 && (selectedIndustry && selectedIndustry.current_interns.length >= selectedIndustry.quota)">
                            <div
                                class="p-5 sm:p-6 rounded-3xl bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-100 dark:border-yellow-500/20 text-yellow-800 dark:text-yellow-400 text-sm flex items-start gap-3 sm:gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="leading-relaxed opacity-90">Industri ini sudah mencapai batas kuota maksimal.
                                        Anda tidak dapat menambahkan siswa lagi.</p>
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <!-- Modal Footer -->
                    <div
                        class="p-5 sm:p-8 border-t border-gray-100 dark:border-amoled-border bg-gray-50/50 dark:bg-white/[0.01] flex items-center justify-between flex-shrink-0">
                        <div class="flex flex-col">
                            <p
                                class="text-[10px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.2em] mb-1">
                                Status Pilihan</p>
                            <p class="text-sm font-bold text-gray-900 dark:text-white">
                                Terpilih: <span class="text-school-blue" x-text="selectedStudents.length"></span> <span
                                    class="text-gray-400">/</span> <span x-text="limit"></span> Siswa
                            </p>
                        </div>
                        ...
                        <form action="{{ route('placements.store') }}" method="POST">
                            ...
                            <input type="hidden" name="industry_id"
                                :value="selectedIndustry ? selectedIndustry.id : ''">
