...
@if ($journal->attachment_path)
    <div>
        <p class="text-[10px] font-black text-gray-400 dark:text-amoled-text uppercase tracking-[0.15em] mb-1.5">Lampiran
        </p>
        @if (preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', $journal->attachment_path))
            <img src="{{ asset('storage/' . $journal->attachment_path) }}" alt="Lampiran"
                class="h-32 object-cover rounded-xl border border-gray-200 dark:border-amoled-border" loading="lazy"
...
@if ($journal->verified_at)
    <p class="text-[10px] text-gray-400 dark:text-gray-500">Divalidasi:
        {{ $journal->verified_at->format('d M Y, H:i') }}</p>
@endif
