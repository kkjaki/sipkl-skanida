...
@section('content')
    @php
        $attendanceMap = [
            ...
            'excused' => [
                'label' => 'Izin',
                'class' => 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-400',
            ],
            'sick' => [
                'label' => 'Sakit',
    ...
    <div x-data="{
        ...
        {{-- Student Info Card --}}
        <div
            class="bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-2xl p-4 sm:p-5">
            <div class="flex items-center gap-3">
                <div
                    class="w-11 h-11 rounded-xl bg-school-blue/10 flex items-center justify-center text-school-blue font-black text-base flex-shrink-0">
                    {{ substr($internship->student->user->name, 0, 1) }}
                </div>
                <div class="min-w-0">
                    <h1 class="text-lg font-bold text-gray-900 dark:text-white truncate">
                        {{ $internship->student->user->name }}</h1>
                    <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ $internship->student->class_name }}
                        &bull; {{ $internship->student->nis }} &bull; {{ $internship->industry->name ?? '-' }}</p>
                </div>
            </div>
        </div>
        ...
        {{-- Toolbar --}}
        <div
            class="bg-white dark:bg-amoled-surface border border-gray-100 dark:border-amoled-border rounded-2xl p-3 sm:p-4">
            {{-- Desktop: filters left, actions right --}}
            <div class="hidden sm:flex items-center justify-between gap-4">
                <form method="GET" action="{{ route('supervisor.journal-validations.show', $internship) }}"
                    class="flex items-center gap-2">
                    <select name="status" onchange="this.form.submit()" title="Filter Status"
                    ...
                    <select name="sort" onchange="this.form.submit()" title="Urutkan"
                ...
                <div class="flex items-center gap-2">
                    <form action="{{ route('supervisor.journal-validations.bulkUpdate', $internship) }}" method="POST"
                        ...
                        <button type="submit" :disabled="selectedIds.length === 0"
                            ...
                            Validasi
                            <span x-show="selectedIds.length" x-text="'(' + selectedIds.length + ')'"
        ...
        <div
            ...
            <div class="hidden sm:block overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
                    <thead
                        class="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-amoled-surface dark:text-gray-400 border-b border-gray-200 dark:border-amoled-border">
                        <tr>
                            <th class="px-4 py-3 w-10">
                                <div @click="toggleAll()"
                                    class="w-[18px] h-[18px] rounded border-2 flex items-center justify-center cursor-pointer transition-colors"
                                    :class="selectedIds.length === allIds.length && allIds.length > 0 ?
                                        'bg-school-blue border-school-blue' :
                                        'border-gray-300 dark:border-amoled-border hover:border-school-blue/50'">
                                    <svg x-show="selectedIds.length === allIds.length && allIds.length > 0"
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-amoled-border">
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer"
                                ...
                                <td class="px-4 py-3" @click.stop>
                                    <div @click="toggleSelect({{ $journal->id }})"
                                        class="w-[18px] h-[18px] rounded border-2 flex items-center justify-center cursor-pointer transition-colors"
                                        :class="selectedIds.includes({{ $journal->id }}) ?
                                            'bg-school-blue border-school-blue' :
                                            'border-gray-300 dark:border-amoled-border hover:border-school-blue/50'">
                                        <svg x-show="selectedIds.includes({{ $journal->id }})"
                                            class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                                d="M5 13l4 4L19 7" />
                                ...
                                <td class="px-4 py-3 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                    {{ $journal->date->format('d M Y') }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span
                                        class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $att['class'] }}">{{ $att['label'] }}</span>
                                </td>
                                <td class="px-4 py-3 max-w-[250px]">
                                    <span
                                        class="text-gray-700 dark:text-gray-300 truncate block">{{ Str::limit($journal->activity, 50) ?: '-' }}</span>
                                </td>
                                ...
                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                    <span
                                        class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $st['class'] }}">{{ $st['label'] }}</span>
                                </td>
                            </tr>
                            {{-- Expanded Content --}}
                        ...
                        @empty
                            <tr>
                                <td colspan="6"
                                    class="px-6 py-12 text-center text-gray-400 dark:text-gray-500 text-sm">Belum ada
                                    jurnal harian.</td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="sm:hidden divide-y divide-gray-100 dark:divide-amoled-border">
                {{-- Select All --}}
                @if ($journals->count() > 0)
                    <div class="px-4 py-2.5 flex items-center gap-3 bg-gray-50/50 dark:bg-white/[0.01]"
                        @click="toggleAll()">
                        <div class="w-[18px] h-[18px] rounded border-2 flex items-center justify-center cursor-pointer transition-colors flex-shrink-0"
                            :class="selectedIds.length === allIds.length && allIds.length > 0 ?
                                'bg-school-blue border-school-blue' : 'border-gray-300 dark:border-amoled-border'">
                            <svg x-show="selectedIds.length === allIds.length && allIds.length > 0"
                        ...
                        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400"
                            x-text="selectedIds.length === allIds.length && allIds.length > 0 ? 'Batalkan Semua' : 'Pilih Semua'">Pilih
                            Semua</span>
                    </div>
                @endif
                @forelse($journals as $journal)
                    ...
                    <div class="px-4 py-3">
                        <div class="flex items-start gap-3" @click="toggleExpand({{ $journal->id }})">
                            ...
                            <div @click.stop="toggleSelect({{ $journal->id }})"
                                ...
                                :class="selectedIds.includes({{ $journal->id }}) ? 'bg-school-blue border-school-blue' :
                                    'border-gray-300 dark:border-amoled-border'">
                                <svg x-show="selectedIds.includes({{ $journal->id }})" class="w-2.5 h-2.5 text-white"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2 mb-1">
                                    <span
                                        class="text-sm font-bold text-gray-900 dark:text-white">{{ $journal->date->format('d M Y') }}</span>
                                    <span
                                        class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $st['class'] }} flex-shrink-0">{{ $st['label'] }}</span>
                                </div>
                                <div class="flex items-center gap-2 mb-1.5">
                                    <span
                                        class="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase {{ $att['class'] }}">{{ $att['label'] }}</span>
                                    @if ($journal->attachment_path)
                                        <span class="text-[10px] font-medium text-school-blue">📎 Ada</span>
                                    @endif
                                </div>
                                <p class="text-xs text-gray-600 dark:text-gray-400 line-clamp-1">
                                    {{ Str::limit($journal->activity, 60) ?: '-' }}</p>
                            </div>

                            {{-- Expand Arrow --}}
                            <svg class="w-4 h-4 text-gray-400 flex-shrink-0 mt-1 transition-transform"
                                :class="expandedRow === {{ $journal->id }} ? 'rotate-180' : ''" fill="none"
            ...
            @if ($journals->hasPages())
                <div
                    class="px-4 py-3 border-t border-gray-200 dark:border-amoled-border bg-gray-50 dark:bg-amoled-surface">
                    {{ $journals->links() }}
                </div>
            @endif
        </div>
        ...
        {{-- Reject Modal --}}
        <div x-show="showRejectModal" class="fixed inset-0 z-[100]" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center justify-center min-h-screen p-4 sm:p-6">
                ...
                <div class="relative w-full sm:max-w-md bg-white dark:bg-amoled-surface rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-amoled-border"
                    ...
                    <form action="{{ route('supervisor.journal-validations.bulkUpdate', $internship) }}" method="POST">
                        ...
                        <div
                            class="p-4 sm:p-5 border-b border-gray-100 dark:border-amoled-border flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400"><span
                                        x-text="selectedIds.length"></span> jurnal akan ditolak</p>
                            </div>
                            <button type="button" @click="closeRejectModal()"
                                class="p-1.5 rounded-lg bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/20 transition-colors flex-shrink-0">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                        d="M6 18L18 6M6 6l12 12" />
                                </svg>
                        ...
                        <div class="p-4 sm:p-5">
                            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">Alasan Penolakan
                                <span class="text-red-500">*</span></label>
                            <textarea name="rejection_note" rows="3" required placeholder="Tuliskan alasan mengapa jurnal ditolak..."
                        ...
                        <div
                            class="p-4 sm:p-5 border-t border-gray-100 dark:border-amoled-border bg-gray-50/50 dark:bg-white/[0.01] grid grid-cols-2 gap-2">
                            <button type="button" @click="closeRejectModal()"
