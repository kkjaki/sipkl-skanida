    ...
    <div class="flex flex-col gap-6" x-data="{ showErrorModal: false }">
        ...
        @elseif(!$hasScores)
            ...
            <div
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-sm text-center mt-1">Selesaikan magang dan pastikan
                    nilai telah diinput oleh Guru Pembimbing sebelum melakukan validasi data sertifikat.</p>
            </div>
        @else
            ...
            <div
                ...
                <div class="border-b border-gray-200 dark:border-amoled-border py-4 px-5 sm:px-8">
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Data di bawah ini akan tercetak pada
                        sertifikat Anda.</p>
                </div>
                ...
                <div class="p-5 sm:p-8">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5">
                        ...
                        <div>
                            <dt
                                class="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">
                                Nama Lengkap</dt>
                            <dd class="text-sm font-semibold text-gray-800 dark:text-white">
                                {{ $internship->student->user->name ?? '-' }}</dd>
                        </div>
                        ...
                        <div>
                            <dt
                                class="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">
                                NIS</dt>
                            <dd class="text-sm font-semibold text-gray-800 dark:text-white font-mono">
                                {{ $internship->student->nis ?? '-' }}</dd>
                        </div>
                        ...
                        <div>
                            ...
                            <dd class="text-sm font-semibold text-gray-800 dark:text-white">
                                {{ $internship->student->place_of_birth }},
                                {{ \Carbon\Carbon::parse($internship->student->date_of_birth)->locale('id')->translatedFormat('d F Y') }}
                            </dd>
                        </div>
                        ...
                        <div>
                            <dt
                                ...
                                Kelas</dt>
                            <dd class="text-sm font-semibold text-gray-800 dark:text-white">
                                {{ $internship->student->class_name ?? '-' }}</dd>
                        </div>
                        ...
                        <div>
                            <dt
                                class="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">
                                Penanggung Jawab (PIC)</dt>
                            <dd class="text-sm font-semibold text-gray-800 dark:text-white">
                                {{ $internship->industry->pic_name ?? '-' }}</dd>
                        </div>
            ...
            @if ($certificate->status === 'draft')
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    {{-- Validate Button --}}
                    <form action="{{ route('student.certificate-validations.validate', $certificate) }}" method="POST"
                        class="flex-1 sm:flex-initial"
                        @submit.prevent="$store.confirmModal.show({ title: 'Validasi Data Sertifikat', message: 'Apakah Anda yakin data di atas sudah benar? Tindakan ini tidak dapat dibatalkan.', type: 'warning', confirmText: 'Ya, Data Sudah Benar', onConfirm: () => $el.submit() })">
                        @csrf
                        @method('PUT')
                        <button type="submit"
                    ...
                    {{-- Report Error Button --}}
                    <button type="button" @click="showErrorModal = true"
            ...
            {{-- Error Report Modal --}}
            <div x-show="showErrorModal" x-cloak x-transition.opacity.duration.200ms
                ...
                <div x-show="showErrorModal" x-transition:enter="transition ease-out duration-200"
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed mb-6">
                        Silakan hubungi bagian <strong class="text-gray-800 dark:text-white">Admin/Humas</strong> untuk
                        memperbaiki data Anda pada sistem. Pastikan untuk menyampaikan data mana yang perlu diperbaiki.
                    </p>
