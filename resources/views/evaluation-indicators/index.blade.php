    ...
    <div x-data="{
        ...
        @if ($indicators->isEmpty())
            <div
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center">Klik tombol "Tambah Indikator"
                    untuk menambahkan indikator penilaian baru.</p>
            </div>
        @else
            <div
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-amoled-border">
                            @foreach ($indicators as $index => $indicator)
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                    <td class="px-5 py-3.5 font-medium text-gray-900 dark:text-white">{{ $index + 1 }}
                                    </td>
                                    <td class="px-5 py-3.5 text-gray-700 dark:text-gray-300">{{ $indicator->name }}</td>
                                    <td class="px-5 py-3.5 text-center">
                                        <div class="flex items-center justify-center gap-1.5">
                                            <button
                                                @click="openEdit({{ $indicator->id }}, '{{ addslashes($indicator->name) }}')"
                                                type="button"
                                                class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-school-blue hover:bg-school-blue/10 dark:hover:bg-school-blue/20 transition-colors"
                                                title="Edit">
                                            ...
                                            <form action="{{ route('evaluation-indicators.destroy', $indicator) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-500/15 transition-colors"
                                                    title="Hapus">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                    ...
                    @foreach ($indicators as $index => $indicator)
                        <div class="px-4 py-3 flex items-center justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <span
                                    class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ $indicator->name }}</span>
                            </div>
                            <div class="flex items-center gap-1 flex-shrink-0">
                                <button @click="openEdit({{ $indicator->id }}, '{{ addslashes($indicator->name) }}')"
                                    type="button"
        ...
        {{-- Add/Edit Modal --}}
        <div x-show="showModal" class="fixed inset-0 z-[100]" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div
                            ...
                            <div>
                                <h2 class="text-base font-bold text-gray-900 dark:text-white"
                                    x-text="editMode ? 'Edit Indikator' : 'Tambah Indikator'"></h2>
                                <p class="text-xs text-gray-500 dark:text-gray-400"
                                    x-text="editMode ? 'Ubah nama indikator penilaian' : 'Masukkan nama indikator penilaian baru'">
                                </p>
                            </div>
