        ...
        @if(!$activeYear)
            <div class="flex w-full border-l-4 border-amber-500 bg-white dark:bg-amoled-surface px-4 py-3 shadow-sm rounded-r-xl">
                <div class="w-full flex items-center gap-2">
                    ...
                    <p class="text-sm text-amber-600 dark:text-amber-400 font-medium">Belum ada tahun ajaran aktif. Hubungi Admin untuk mengaktifkan tahun ajaran.</p>
                </div>
            </div>
        @endif
            ...
            <a href="{{ route('supervisors.allocate') }}" class="block group">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-amoled-border dark:bg-amoled-surface transition hover:border-violet-300 dark:hover:border-violet-500/40 hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-500 dark:text-amoled-text">Guru Pembimbing Teralokasi</p>
                            <h3 class="mt-2 text-3xl font-bold text-violet-600 dark:text-violet-400">
                                {{ $totalSupervisorsAllocated }} <span class="text-base font-medium text-gray-400 dark:text-gray-500">/ {{ $totalSupervisors }}</span>
                            </h3>
                            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">guru pembimbing siap membimbing siswa</p>
                        </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-amoled-border dark:bg-amoled-surface transition hover:border-red-300 dark:hover:border-red-500/40 hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="mt-2 text-3xl font-bold {{ $studentsWithoutScores > 0 ? 'text-red-500' : 'text-emerald-500' }}">
                                {{ $studentsWithoutScores }}
                            </h3>
                            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">internship tanpa nilai PKL</p>
                        </div>
                        <div class="flex h-12 w-12 items-center justify-center rounded-xl {{ $studentsWithoutScores > 0 ? 'bg-red-500/10 dark:bg-red-500/20' : 'bg-emerald-500/10 dark:bg-emerald-500/20' }} shrink-0">
                            <svg class="w-6 h-6 {{ $studentsWithoutScores > 0 ? 'text-red-500' : 'text-emerald-500' }}" width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('evaluation-indicators.index') }}" class="block group">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-amoled-border dark:bg-amoled-surface transition hover:border-amber-300 dark:hover:border-amber-500/40 hover:shadow-md">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-amber-500/10 dark:bg-amber-500/20 shrink-0">
