            ...
            <div
                ...
                <div class="w-full flex items-center gap-2">
                    ...
                    <p class="text-sm text-amber-600 dark:text-amber-400 font-medium">
                        Belum ada tahun ajaran aktif. Silakan aktifkan tahun ajaran terlebih dahulu di menu <a
                            href="{{ route('academic-years.index') }}" class="underline hover:text-amber-700">Tahun
                            Ajaran</a>.
                    </p>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3
                            class="mt-2 text-3xl font-bold {{ $stats['students_unplaced'] > 0 ? 'text-red-500' : 'text-gray-400 dark:text-gray-500' }}">
                            {{ number_format($stats['students_unplaced']) }}
                        </h3>
                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">siswa belum PKL</p>
                    </div>
                    <div
                        class="flex h-12 w-12 items-center justify-center rounded-xl {{ $stats['students_unplaced'] > 0 ? 'bg-red-500/10 dark:bg-red-500/20' : 'bg-gray-100 dark:bg-white/[0.06]' }} shrink-0">
                        <svg class="w-6 h-6 {{ $stats['students_unplaced'] > 0 ? 'text-red-500' : 'text-gray-400 dark:text-gray-500' }}"
                ...
                <div class="p-6 sm:p-8">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($stats['students_per_dept'] as $dept)
                            ...
                            @endphp
                            <div
                                class="flex items-center gap-4 rounded-xl border border-gray-100 dark:border-amoled-border p-4 transition hover:bg-gray-50 dark:hover:bg-white/[0.03]">
                                <div
                                    class="flex h-10 w-10 items-center justify-center rounded-lg {{ $iconBg }} shrink-0">
                                    <span class="text-sm font-bold {{ $textColor }}">{{ $dept->code }}</span>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-gray-800 dark:text-white truncate">
                                        {{ $dept->name }}</p>
                                    <p class="text-xs text-gray-400 dark:text-gray-500">{{ $dept->total }} peserta didik
                                    </p>
                                </div>
