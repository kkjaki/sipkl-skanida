...
@section('content')
    <div class="flex flex-col gap-6">
                ...
                <div
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3
                                class="mt-2 text-3xl font-bold {{ $studentsUnassessed > 0 ? 'text-red-500' : 'text-emerald-500' }}">
                                {{ $studentsUnassessed }}</h3>
                            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">siswa belum ada nilai</p>
                        </div>
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-xl {{ $studentsUnassessed > 0 ? 'bg-red-500/10 dark:bg-red-500/20' : 'bg-emerald-500/10 dark:bg-emerald-500/20' }} shrink-0">
                            <svg class="w-6 h-6 {{ $studentsUnassessed > 0 ? 'text-red-500' : 'text-emerald-500' }}"
                                width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
            ...
            <div
                ...
                <div
                    ...
                    <a href="{{ route('supervisor.journal-validations.index') }}"
                        class="text-xs font-medium text-school-blue hover:underline">Kelola Jurnal →</a>
                </div>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-amoled-border">
                                <th class="py-3 px-6 sm:px-8 font-medium text-gray-500 dark:text-amoled-text">Nama Siswa
                                </th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.03] transition duration-150">
                                    <td class="py-3.5 px-6 sm:px-8">
                                        <p class="font-medium text-gray-800 dark:text-white">
                                            {{ $internship->student->user->name ?? '-' }}</p>
                                        <p class="text-xs text-gray-400 dark:text-gray-500">
                                            {{ $internship->student->class_name ?? '-' }}</p>
                                    </td>
                                    <td class="py-3.5 px-4">
                                        <p class="text-gray-700 dark:text-gray-300">
                                            {{ $internship->industry->name ?? '-' }}</p>
                                        <p class="text-xs text-gray-400 dark:text-gray-500">
                                            {{ $internship->industry->city ?? '' }}</p>
                                    </td>
                                    <td class="py-3.5 px-4">
                                        @php
                                            $pending = $internship->dailyJournals
                    ...
                    @foreach ($internships as $internship)
                        ...
                        <div class="p-4 space-y-2">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center rounded-lg bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-500 shrink-0">
                                        Dinilai ✓
                                    </span>
                                @endif
                            </div>
        ...
        @else
            <div
                class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-amoled-border dark:bg-amoled-surface p-10 text-center">
                <div
                    class="flex h-16 w-16 items-center justify-center rounded-2xl bg-school-blue/10 dark:bg-school-blue/20 mx-auto mb-4">
                    <svg class="w-8 h-8 text-school-blue" width="32" height="32" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Anda belum dialokasikan ke siswa manapun untuk
                    tahun ajaran ini.</p>
            </div>
        @endif
    </div>
