    ...
    <div class="flex flex-col gap-6" x-data="{
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div class="relative sm:max-w-[220px] sm:min-w-[160px]">
                <select x-model="selectedClass" aria-label="Filter Kelas"
            ...
            {{-- Reset --}}
            <button x-show="search !== '' || selectedClass !== ''" x-cloak @click="search = ''; selectedClass = ''"
                type="button"
                ...
                <table class="w-full table-auto">
                    <thead class="sticky top-0 z-10">
                        <tr
                            ...
                            <th
                                class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-center w-32">
                            ...
                            <th
                                class="py-3.5 px-4 text-sm font-semibold text-gray-500 dark:text-amoled-text text-center xl:pr-8 w-28">
                                Nilai Akhir
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($internships as $index => $internship)
                            @php
                                ...
                                $finalScore =
                                    $avgIndustry !== null && $avgSchool !== null
                                        ? round(($avgIndustry + $avgSchool) / 2, 2)
                                        : null;
                            @endphp
                            <tr data-name="{{ strtolower($studentName) }}" data-nis="{{ $studentNis }}"
...
&&
                                ...
                                <td class="py-4 px-4 text-center">
                                    @if ($avgIndustry !== null)
                                        ...
                                        @endphp
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border {{ $colorIndustry }}">
                                            {{ number_format($avgIndustry, 2) }}
                                        </span>
                                    @else
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border bg-gray-100 text-gray-400 border-gray-200 dark:bg-white/[0.06] dark:text-gray-500 dark:border-amoled-border">
                                            Belum Dinilai
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="py-4 px-4 text-center">
                                    @if ($avgSchool !== null)
                                        ...
                                        @endphp
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-0.5 text-xs font-semibold border {{ $colorSchool }}">
                                ...
                                <td class="py-4 px-4 xl:pr-8 text-center">
                                    @if ($finalScore !== null)
                                        ...
                                        @endphp
                                        <span
                                            class="inline-block rounded-lg px-2.5 py-1 text-sm font-bold border {{ $colorFinal }}">
                    ...
                    @php
                        ...
                        $finalScore =
                            $avgIndustry !== null && $avgSchool !== null
                                ...
                                : null;
                    @endphp
                    <div class="p-4" data-name="{{ strtolower($studentName) }}" data-nis="{{ $studentNis }}"
                        data-class="{{ $className }}"
                        x-show="
                            (selectedClass === '' || $el.dataset.class === selectedClass)
&&
                        ...
                        <div class="flex items-start justify-between mb-2">
                            ...
                            <span
                                class="text-xs px-2.5 py-0.5 rounded-lg border font-semibold ml-2 whitespace-nowrap bg-blue-500/10 text-blue-500 border-blue-500/20">{{ $className }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <div class="text-center">
                                <span
                                    class="block text-[10px] text-gray-400 dark:text-gray-500 uppercase mb-1">Industri</span>
                                @if ($avgIndustry !== null)
                                    @php
                                        $colorIndustry = match (true) {
                                            $avgIndustry >= 90
                                    ...
                                    @endphp
                                    <span
                                        class="inline-block rounded-lg px-2 py-0.5 text-xs font-semibold border {{ $colorIndustry }}">{{ number_format($avgIndustry, 2) }}</span>
                                @else
                                    <span
                                        class="inline-block rounded-lg px-2 py-0.5 text-[10px] font-semibold border bg-gray-100 text-gray-400 border-gray-200 dark:bg-white/[0.06] dark:text-gray-500 dark:border-amoled-border">N/A</span>
                                @endif
                            </div>
                            {{-- Sekolah --}}
                            <div class="text-center">
                                <span
                                    class="block text-[10px] text-gray-400 dark:text-gray-500 uppercase mb-1">Sekolah</span>
                                @if ($avgSchool !== null)
                                    @php
                                        $colorSchool = match (true) {
                                            $avgSchool >= 90
                                    ...
                                    @endphp
                                    <span
                                        class="inline-block rounded-lg px-2 py-0.5 text-xs font-semibold border {{ $colorSchool }}">{{ number_format($avgSchool, 2) }}</span>
                                @else
                                    <span
                                        class="inline-block rounded-lg px-2 py-0.5 text-[10px] font-semibold border bg-gray-100 text-gray-400 border-gray-200 dark:bg-white/[0.06] dark:text-gray-500 dark:border-amoled-border">N/A</span>
                                @endif
                            ...
                            <div class="text-center">
                                <span
                                    class="block text-[10px] text-gray-400 dark:text-gray-500 uppercase mb-1">Akhir</span>
                                @if ($finalScore !== null)
                                    @php
                                        $colorFinal = match (true) {
                                            $finalScore >= 90
                                    ...
                                    @endphp
                                    <span
                                        class="inline-block rounded-lg px-2 py-0.5 text-sm font-bold border {{ $colorFinal }}">{{ number_format($finalScore, 2) }}</span>
                                @else
                                    <span
                                        class="inline-block rounded-lg px-2 py-0.5 text-[10px] font-semibold border bg-gray-100 text-gray-400 border-gray-200 dark:bg-white/[0.06] dark:text-gray-500 dark:border-amoled-border">N/A</span>
                                @endif
