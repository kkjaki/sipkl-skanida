<div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-amoled-border dark:bg-amoled-surface">
    <div class="p-5 sm:p-6">
        <div class="mb-5">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui informasi profil dan alamat email akun
                Anda.</p>
            @if ($user->hasRole('student'))
                <p class="mt-2 text-xs text-gray-400 dark:text-gray-500">
                    ...
                    Data nama, program keahlian, dan kelas tidak dapat diubah sendiri oleh siswa. Hubungi Admin jika
                    terdapat kesalahan.
                </p>
            @endif
        </div>
        ...
        <form method="post" action="{{ route('profile.update') }}" class="space-y-5">
            ...
            <div>
                <label for="name"
                    class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nama</label>
                @if ($user->hasRole('student'))
                ...
                @else
                    <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}" required
                        class="w-full rounded-xl border border-gray-200 dark:border-amoled-border bg-gray-50 dark:bg-white/[0.03] text-gray-900 dark:text-white p-2.5 text-sm focus:ring-2 focus:ring-school-blue/20 focus:border-school-blue transition-colors">
                    @error('name')
            ...
            @if ($user->hasRole('student') && $user->student)
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Program
                            Keahlian</label>
                    ...
                    <div class="mt-2">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <button form="send-verification"
                                class="underline text-sm text-school-blue hover:text-school-blue/80 font-medium transition-colors">
