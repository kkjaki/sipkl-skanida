        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-amoled-border dark:bg-amoled-surface">
            <div class="p-4 sm:p-6">
                <form action="{{ route('verification.index') }}" method="GET" class="flex gap-2">
                    ...
                    <button type="submit"
                        class="h-11 inline-flex items-center justify-center rounded-lg bg-school-blue px-5 text-sm font-medium text-white hover:bg-school-blue/90 transition duration-150 shadow-sm">
                        Cari
                    </button>
                </form>
            </div>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="border-b border-gray-200 dark:border-amoled-border">
                                <th class="py-3.5 px-6 sm:px-8 font-medium text-gray-500 dark:text-amoled-text">Nama
                                    Industri</th>
                                ...
                                <th class="py-3.5 px-6 sm:px-8 font-medium text-gray-500 dark:text-amoled-text text-right">
                                    Aksi</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.03] transition duration-150">
                                    ...
                                    <td class="py-3.5 px-4">
                                        @if ($proposal->is_synced)
                                            <span
                                                class="inline-flex items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                                Sudah Sync
                                            </span>
                                        @elseif($proposal->status === 'blacklisted')
                                            <span
                                                class="inline-flex items-center gap-1 rounded-full bg-red-50 dark:bg-red-500/10 px-2.5 py-1 text-xs font-medium text-red-600 dark:text-red-400">
                                                <span class="w-1.5 h-1.5 rounded-full bg-red-500"></span>
                                                Ditolak
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 text-xs font-medium text-amber-600 dark:text-amber-400">
                    ...
                    @foreach ($proposals as $proposal)
                        <div class="p-4 space-y-2">
                            <div class="flex items-start justify-between gap-2">
                                ...
                                @if ($proposal->is_synced)
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-400 shrink-0">Sync</span>
                                @elseif($proposal->status === 'blacklisted')
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-red-50 dark:bg-red-500/10 px-2 py-1 text-xs font-medium text-red-600 dark:text-red-400 shrink-0">Ditolak</span>
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-500/10 px-2 py-1 text-xs font-medium text-amber-600 dark:text-amber-400 shrink-0">Menunggu</span>
                                @endif
                            </div>
                            <div class="flex items-center justify-between">
                                <p class="text-xs text-gray-500 dark:text-gray-400">Oleh: <span
            ...
            @else
                <div class="p-8 text-center">
                    ...
                    <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                        {{ $search ? 'Tidak ada pengajuan yang cocok dengan pencarian.' : 'Belum ada pengajuan industri dari siswa di program keahlian Anda.' }}
                    </p>
                </div>
            @endif
        </div>
