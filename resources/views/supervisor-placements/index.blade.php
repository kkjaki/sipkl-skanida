    ...
    <div x-data="{
                ...
                <div
                    ...
                    <div class="p-5 sm:p-6 flex-1">
                        <div class="flex justify-between items-start gap-3 mb-4 sm:mb-6">
                            <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-school-blue/10 flex items-center justify-center text-school-blue font-black text-lg sm:text-xl flex-shrink-0"
                                x-text="supervisor.user.name.charAt(0)"></div>
                            <span
                                :class="supervisor.remaining_quota > 0 ?
                        ...
                        <h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-1 leading-tight"
                            x-text="supervisor.user.name"></h3>
                        <p
                            class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-4 sm:mb-6 flex items-center gap-1.5">
                        ...
                        <div
                            ...
                            <div class="text-center flex-1 border-r border-gray-100 dark:border-amoled-border">
                                ...
                                <p class="text-base sm:text-lg font-bold text-gray-900 dark:text-white"
                                    x-text="supervisor.interns_count"></p>
                            </div>
        ...
        <template x-if="supervisors.length === 0">
            <div
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center">Belum ada guru pembimbing yang
        ...
        <template x-if="supervisors.length > 0 && filteredSupervisors.length === 0">
            <div
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 max-w-xs text-center">Tidak ada guru yang cocok dengan
                    pencarian atau filter Anda.</p>
            </div>
        </template>
            ...
            <div class="flex items-center justify-center min-h-screen p-4 sm:p-6">
                ...
                <div class="relative w-full sm:max-w-2xl bg-white dark:bg-amoled-surface rounded-2xl sm:rounded-[2rem] shadow-2xl overflow-hidden border border-gray-200 dark:border-amoled-border max-h-[90vh] flex flex-col"
                    ...
                    <div
                        ...
                        <div class="min-w-0 flex-1">
                            <h2 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate"
                                x-text="selectedSupervisor ? selectedSupervisor.user.name : ''"></h2>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1"
                                x-text="selectedSupervisor && selectedSupervisor.nip ? 'NIP: ' + selectedSupervisor.nip : ''">
                            </p>
                    ...
                    <div class="p-5 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                            ...
                            <div class="flex items-center justify-between mb-3 sm:mb-4">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        ...
                                        x-text="selectedSupervisor ? selectedSupervisor.current_interns.length : 0"></span>
                                </div>
                            </div>

                            <!-- Select All (bulk delete mode) -->
                            <div x-show="deleteMode && selectedSupervisor && selectedSupervisor.current_interns.length > 1"
                                class="mb-3" x-cloak>
                                <label @click="toggleAllInterns()"
                                    ...
                                    <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-all"
                                        :class="selectedSupervisor && selectedInterns.length === selectedSupervisor
                                            .current_interns.length ? 'bg-red-500 border-red-500' :
                                            'border-gray-300 dark:border-amoled-border'">
                                        <svg x-show="selectedSupervisor && selectedInterns.length === selectedSupervisor.current_interns.length"
                                            ...
                                            <div class="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    <p class="text-[11px] font-medium text-gray-500 dark:text-gray-400 truncate"
                                                        x-text="intern.student.class_name + (intern.industry ? ' • ' + intern.industry.name : '')">
                                                    </p>
                                                </div>
                                            </div>
                                ...
                                <template x-if="!selectedSupervisor || selectedSupervisor.current_interns.length === 0">
                                    <div
                                        ...
                                        <p class="text-sm text-gray-400 dark:text-gray-500 italic font-medium">Belum ada
                                            siswa yang dibimbing oleh guru ini.</p>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-2 sm:gap-3">
                                <template x-for="candidate in filteredCandidates" :key="candidate.id">
                                    <label
                                        ...
                                        <input type="checkbox" class="sr-only" :value="candidate.id"
                                            :checked="selectedInternships.includes(candidate.id)"
                                            :disabled="isDisabled(candidate.id)" @change="toggleInternship(candidate.id)">
                                        ...
                                        <div class="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                                            <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 dark:text-gray-400 font-black text-sm sm:text-base flex-shrink-0"
                                                :class="selectedInternships.includes(candidate.id) ?
                                                    'bg-school-blue/20 text-school-blue' : ''"
                                                x-text="candidate.student.user.name.charAt(0)"></div>
                                            <div class="min-w-0">
                                                <p class="text-sm font-bold text-gray-900 dark:text-white truncate"
                                                    x-text="candidate.student.user.name"></p>
                                                <p class="text-[11px] font-medium text-gray-500 dark:text-gray-400 truncate"
                                                    x-text="candidate.student.class_name + (candidate.industry ? ' • ' + candidate.industry.name : '')">
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 flex-shrink-0"
                                            ...
                                            <svg x-show="selectedInternships.includes(candidate.id)"
                                ...
                                <template x-if="candidates.length === 0">
                                    <div
                                        ...
                                        <p class="text-sm text-gray-400 dark:text-gray-500 italic font-medium">Semua siswa
                                            sudah memiliki guru pembimbing.</p>
                                    </div>
                                </template>
                        ...
                        <template
                            x-if="limit <= 0 && (selectedSupervisor && selectedSupervisor.current_interns.length >= selectedSupervisor.quota)">
                            <div
                                ...
                                <div>
                                    ...
                                    <p class="leading-relaxed opacity-90">Guru pembimbing ini sudah mencapai batas kuota
                                        maksimal. Anda tidak dapat menambahkan siswa lagi.</p>
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <div
                        ...
                        <div class="flex flex-col">
                            ...
                            <p class="text-sm font-bold text-gray-900 dark:text-white">
                                Terpilih: <span class="text-school-blue" x-text="selectedInternships.length"></span> <span
                        ...
                        <form action="{{ route('supervisor-placements.store') }}" method="POST">
                            ...
                            <input type="hidden" name="supervisor_id"
                                :value="selectedSupervisor ? selectedSupervisor.user_id : ''">
                            <template x-for="id in selectedInternships" :key="'form-' + id">
                                <input type="hidden" name="internship_ids[]" :value="id">
                            </template>
                            <button type="submit" :disabled="selectedInternships.length === 0"
